...
const FAUCET_ADDRESS = process.env
  .NEXT_PUBLIC_FAUCET_CONTRACT_ADDRESS as `0x${string}`;

// --- Konfigurasi untuk Kontrak Multisender ---
const MULTISENDER_CONTRACT_ADDRESS =
  (process.env.NEXT_PUBLIC_MULTISENDER_CONTRACT_ADDRESS as `0x${string}`) ||
...
export default function AdminPage() {
  ...
  const [multisenderWithdrawRecipient, setMultisenderWithdrawRecipient] =
    useState("");
  const [multisenderDepositAmount, setMultisenderDepositAmount] = useState("");

  // --- Data Faucet ---
  const { data: faucetBalance, refetch: refetchFaucetBalance } = useBalance({
    address: FAUCET_ADDRESS,
  });

  const { data: currentClaimAmount, refetch: refetchClaimAmount } =
    useReadContract({
      abi: abiFaucet,
      address: FAUCET_ADDRESS,
  ...
  // --- Data Multisender ---
  const { data: multisenderBalance, refetch: refetchMultisenderBalance } =
    useBalance({ address: MULTISENDER_CONTRACT_ADDRESS });

  // Membaca pemilik kontrak Multisender (jika menggunakan Ownable)
  const { data: multisenderOwner, refetch: refetchMultisenderOwner } =
    useReadContract({
      abi: abiMultiSender,
  ...
  const formatDuration = (
    totalSeconds: number | bigint | undefined | null
  ): string => {
  ...
  const isOwner =
    ...
    (address?.toLowerCase() ===
      (multisenderOwner as string | undefined)?.toLowerCase() ||
  ...
  const handleMultisenderDeposit = async () => {
    if (
      !multisenderDepositAmount ||
      parseFloat(multisenderDepositAmount) <= 0
    ) {
    ...
    if (
      ...
      toast.error(
        "Multisender contract address is not configured. Cannot deposit.",
        { duration: 5000 }
      );
      return;
    }
    ...
    try {
      ...
      console.error(error);
      toast.error(
        `Multisender deposit failed: ${error.shortMessage || error.message}`,
        { id: toastId }
      );
    }
  };
  ...
  const handleWithdrawStuckFunds = async () => {
    ...
    if (
      ...
      toast.error("Please enter a valid recipient address for withdrawal.", {
        duration: 5000,
      });
      return;
    }
    if (
      ...
      toast.error(
        "Multisender contract address is not configured. Cannot withdraw.",
        { duration: 5000 }
      );
      return;
    }
    ...
    try {
      ...
      console.error(error);
      toast.error(
        `Multisender withdrawal failed: ${error.shortMessage || error.message}`,
        { id: toastId }
      );
    }
  };
    ...
    return (
      <PixelatedCard className="text-center">
        ...
        <p className="mt-2 text-sm text-brand-gray">
          Connected as: {address || "N/A"}
        </p>
        {multisenderOwner && typeof multisenderOwner === "string" ? (
          <p className="mt-2 text-xs text-stone-500">
            Multisender Owner: {multisenderOwner}
          </p>
        ) : null}
        {MULTISENDER_CONTRACT_ADDRESS ===
          "0x0000000000000000000000000000000000000000" && (
          <p className="mt-2 text-xs text-red-400 text-center">
            Multisender contract address is not configured. Please set
            NEXT_PUBLIC_MULTISENDER_CONTRACT_ADDRESS in .env.local
          </p>
        )}
      </PixelatedCard>
    );
    ...
    <div className="max-w-2xl mx-auto space-y-8 p-4 md:p-0">
      ...
      <PixelatedCard>
        ...
        <p className="text-lg">
          ...
          <span className="font-pixel text-white ml-2">
            {faucetBalance
              ? `${formatEther(faucetBalance.value)} ${faucetBalance.symbol}`
              : "Loading..."}
          </span>
        </p>
        <p className="text-lg mt-2">
          ...
          <span className="font-pixel text-white ml-2">
            {typeof currentClaimAmount === "bigint"
              ? `${formatEther(currentClaimAmount)} STT`
              : "Loading..."}
          </span>
        </p>
        <p className="text-lg mt-2">
          ...
          <span className="font-pixel text-white ml-2">
            {formatDuration(
              currentCooldown as bigint | number | undefined | null
            )}
          </span>
        </p>
      </PixelatedCard>
      ...
      <PixelatedCard>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          />
          <PixelatedButton onClick={handleSetClaimAmount}>
            Set Amount
          </PixelatedButton>
        </div>
      </PixelatedCard>
      ...
      <PixelatedCard>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <input
            ...
            className="flex-grow bg-stone-900 text-white p-3 border-2 border-black focus:outline-none focus:border-orange-400"
          />
          <PixelatedButton onClick={handleSetCooldown}>
            Set Cooldown
          </PixelatedButton>
        </div>
      </PixelatedCard>
      ...
      <PixelatedCard>
        ...
        <p className="mb-4">
          This will withdraw the entire balance from the Faucet contract to the
          owner's wallet.
        </p>
        <PixelatedButton
          onClick={handleWithdraw}
          disabled={!faucetBalance || faucetBalance.value === BigInt(0)}
        >
      ...
      <PixelatedCard>
        ...
        <p className="text-lg">
          ...
          <span className="font-pixel text-white ml-2">
            {multisenderBalance
              ? `${formatEther(multisenderBalance.value)} ${
                  multisenderBalance.symbol
                }`
              : "Loading..."}
          </span>
        </p>
        ...
        <p className="mb-4">
          Deposit STT directly to the Multisender contract. These funds will be
          held in the contract until used for multi-send transactions or
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <input
            ...
            className="flex-grow bg-stone-900 text-white p-3 border-2 border-black focus:outline-none focus:border-orange-400"
          />
          <PixelatedButton onClick={handleMultisenderDeposit}>
            Deposit
          </PixelatedButton>
        </div>
        {MULTISENDER_CONTRACT_ADDRESS ===
        ...
        <p className="mb-4">
          This function allows the contract owner to withdraw any STT that got
          stuck in the Multisender contract (e.g., from overpayments or
