...
export default function DeployPage() {
  ...
  const handleDeploy = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const contractToDeploy =
        contractType === "ERC20" ? ERC20Contract : ERC721Contract;

      if (!contractToDeploy.bytecode || contractToDeploy.bytecode === "0x") {
        toast.error(
          "Deployment failed: Contract bytecode is missing or empty. Please check your contract compilation.",
          { id: toastId }
        );
      ...
      if (contractType === "ERC20") {
        const supplyInWei = parseUnits(
          DEFAULT_ERC20_SUPPLY,
      ...
      toast.error(
        `Deployment failed: ${errorMessage}. Please check console for details.`,
        { id: toastId }
      );
    } finally {
      setIsDeploying(false);
  ...
  const isButtonDisabled =
    isDeploying || !isConnected || !tokenName.trim() || !tokenSymbol.trim();
  ...
  return (
    <div className="max-w-2xl mx-auto p-4">
      <PixelatedCard>
        <h1 className="text-3xl text-center mb-6 font-pixel">
          Deploy a Contract
        </h1>
        ...
        <form onSubmit={handleDeploy} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full bg-stone-900 p-3 border-2 border-black text-white focus:outline-none focus:border-orange-400"
            />
          </div>
          <div>
            <label className="block mb-1 font-pixel text-sm">
              Token Symbol
            </label>
            <input
              type="text"
